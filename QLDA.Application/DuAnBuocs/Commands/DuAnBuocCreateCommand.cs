    ...
    private static DuAnBuoc CreateEntity(DuAnBuocCreateDto dto, DanhMucBuoc danhMucBuoc) {
        return new DuAnBuoc {
            ...
            DuAnBuocManHinhs = dto.DanhSachManHinh != null && dto.DanhSachManHinh.Count != 0
                ? [.. dto.DanhSachManHinh.Select((manHinhId, index) => new DuAnBuocManHinh { ManHinhId = manHinhId, Stt = index })]
                : danhMucBuoc.BuocManHinhs?
                .Select(m => new DuAnBuocManHinh { ManHinhId = m.ManHinhId, Stt = m.Stt })
                .ToList() ?? []
        };
    }
